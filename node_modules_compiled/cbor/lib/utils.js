...
exports.extend = function extend() {
  var old = arguments[0];
  var adds = 2 <= arguments.length ? Array.prototype.slice.call(arguments, 1) : [];
  var len = adds.length;
  if (old == null) {
    old = {};
  }
...
exports.DeHexStream = function (_stream$Readable) {
  _inherits(DeHexStream, _stream$Readable);

  function DeHexStream(hex) {
    _classCallCheck(this, DeHexStream);

    var _this = _possibleConstructorReturn(this, _stream$Readable.call(this));
...
exports.HexStream = function (_stream$Transform) {
  _inherits(HexStream, _stream$Transform);

  function HexStream(options) {
    _classCallCheck(this, HexStream);

    return _possibleConstructorReturn(this, _stream$Transform.call(this, options));
  }

  HexStream.prototype._transform = function _transform(fresh, encoding, cb) {
...
exports.streamFiles = function streamFiles(files, streamFunc, cb) {
  ...
  sf.on('error', cb);
  var s = f === '-' ? process.stdin : f instanceof stream.Stream ? f : fs.createReadStream(f);
  s.on('error', cb);
  return s.pipe(sf);
};
